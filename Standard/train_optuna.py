...
def objective(trial, task = None,
    ...
    result_directory = os.path.join(rootDir, "Standard", task)
    ...
    if modelname == "effv2s":
        bs = 16
    dm = ScoutViewDataModule(task = task, addPretrain = True, returnLabel = False, returnAsDict = False, BATCH_SIZE = bs)
    if trial is None:
        # retrain same model
        if final == False:
            callbacks = [model_checkpoint, myEarly_stop_callback, early_stop_callback, progress_bar]
            default_root_dir = os.path.join("../checkpoints", "Standard", task, "best")
            logger = TensorBoardLogger(save_dir=os.path.join("../checkpoints", "Standard", task, "best"), name="logs")
            dm.setup()
        else:
            callbacks = [model_checkpoint, myEarly_stop_callback, early_stop_callback, progress_bar]
            default_root_dir = os.path.join("../checkpoints", "Standard", task, "final")
            logger = TensorBoardLogger(save_dir=os.path.join("../checkpoints", "Standard", task, "final"), name="logs")
            dm.setup("final")
    else:
        callbacks = [model_checkpoint, myEarly_stop_callback, early_stop_callback, progress_bar]
        default_root_dir = os.path.join(rootDir, "Standard/")
        logger = TensorBoardLogger(save_dir=os.path.join(rootDir, "Standard", task), name="logs")
        dm.setup()
    ...
    if trial is None:
        if final == False:
            ...
            test_df.to_csv(f"../results/Standard.{task}.validation.csv")
            shutil.copyfile (model_checkpoint.best_model_path, f"../results/Standard.{task}.best_train.model.ckpt")
            dump(early_stop_callback, f"../results/Standard.{task}.earlystopping.dump")
        else:
            test_df["preds"] = preds
            mae = np.mean(np.abs(test_df["preds"]-test_df["Target"]))
            print ("final MAE on test:", mae)
            test_df = test_df[["preds", "Target"]]
            test_df.to_csv(f"../results/Standard.{task}.test.csv")
            shutil.copyfile (model_checkpoint.best_model_path, f"../results/Standard.{task}.final.model.ckpt")
        return None
    else:
...
if __name__ == "__main__":
    ...
    for task in ["Height", "Weight"]:
        ...
        print(df)
        df.to_csv(f"../results/optuna.results.{task}.Standard.csv")
